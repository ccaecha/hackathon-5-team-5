...
def update_event(id, record):
    updated_count = MockDatabase.update('event', lambda x: x["id"] == id, lambda x: {**x, **record})
    return jsonify({"status": "success", "message": f"{updated_count} event(s) updated successfully"})

@blueprint.route("/event", methods=["DELETE"])
def delete_event(id):
    deleted_count = MockDatabase.delete('event', lambda x: x["id"] == id)
    return jsonify({"status": "success", "message": f"{deleted_count} event(s) deleted successfully"})
...
def update_user(id, record):
    updated_count = MockDatabase.update('user', lambda x: x["id"] == id, lambda x: {**x, **record})
    return jsonify({"status": "success", "message": f"{updated_count} user(s) updated successfully"})

@blueprint.route("/user", methods=["DELETE"])
def delete_user(id):
    deleted_count = MockDatabase.delete('user', lambda x: x["id"] == id)
    return jsonify({"status": "success", "message": f"{deleted_count} user(s) deleted successfully"})
...
def update_calendar(id, record):
    updated_count = MockDatabase.update('calendar', lambda x: x["id"] == id, lambda x: {**x, **record})
    return jsonify({"status": "success", "message": f"{updated_count} calendar(s) updated successfully"})

@blueprint.route("/calendar", methods=["DELETE"])
def delete_calendar(id):
    deleted_count = MockDatabase.delete('calendar', lambda x: x["id"] == id)
    return jsonify({"status": "success", "message": f"{deleted_count} calendar(s) deleted successfully"})
